...
class GameServer:
    ...
    def update_players_on_game(self):
        """ Sends the players the required data to render the game """
        for player in self.players_in_match:
        ...
        for client, packet in packets.items():
            if self.players_data[hash(client)]['LIVES'] > 0:
                if packet.type == "PLAYER_POS":
                    self.players_data[hash(client)]['POS'] = packet.data['POS']
                    self.players_data[hash(client)]['LAST_WALKING_TIME'] = time.time()
                    
                elif packet.type == "PLAYER_ANGLE":
                    self.players_data[hash(client)]['ANGLE'] = packet.data['ANGLE']

                elif packet.type == "PLAYER_SHOOT":
                    print("shoot")
                    self.bullets.append(Bullet(self.players_data[hash(client)]['POS'][:], self.players_data[hash(client)]['ANGLE'], packet.data['PLAYER_MOMENTUM']))
